      ...
      addProbe: (probe) => {
        if (!Object.keys(state.probes).filter(name => name === probe.name).length) {
          const layout = blessed.layout({
            ...
            height: 10,
            top: 10 * Object.keys(state.probes).length,
          ...
          if (probe.type === PROBE_TYPES.distance) { state.probes[probe.name] = Distance(layout, probe.name) }
        }
        
        if (probe.type === PROBE_TYPES.distance) { state.probes[probe.name](probe.distance) }
